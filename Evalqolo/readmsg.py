...
class TrackedPersons:
	def __init__(self, bag, t0):
		self.bag = bag
		self.ids = self.__findids()
		self.lookup = dict.fromkeys(self.ids)
	...
	def __findids(self):
		...
		for topic, msg, t in self.bag.read_messages(topics=['/rwth_tracker/tracked_persons']):
