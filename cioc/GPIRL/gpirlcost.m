...
for i=1:length(infos_pt),
    ...
    % Add to likelihood.
    ll = ll + 0.5*sum(sum(g.*h,3),1) + 0.5*logDet;
    ...
    if nargout > 1,        
        ...
        gradu = gradu + ugrad;
        if gp.learn_ard,
            gradgp.inv_widths = gradgp.inv_widths + gpirlhpxform(gp.inv_widths,iwgrad,gp.ard_xform,2);
        end;
        if gp.learn_rbf,
            gradgp.rbf_var = gradgp.rbf_var + gpirlhpxform(gp.rbf_var,rbfgrad,gp.rbf_xform,2);
        end;
    end;
end;
