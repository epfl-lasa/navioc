% Run the approximate MaxEnt algorithm (LQR variant).
function irl_result = lqrrun(algorithm_params,mdp,mdp_data,features_pt,...
...
theta(end) = auglagfindfeasible(@(wt)lqrcost(wt,infos),length(features),theta);
...
tic;
theta = auglag(@(wt)lqrcost(wt,infos),length(features),theta,options,verbosity);
total_time = toc;
