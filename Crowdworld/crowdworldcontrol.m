...
%At = [eye(Du), eye(Du)*h, zeros(Du); zeros(Du), eye(Du), eye(Du)*h; zeros(Du), zeros(Du), eye(Du)];
%Bt = [zeros(Du); zeros(Du); eye(Du)*h];
At = [eye(Du), eye(Du)*h, eye(Du)*h^2/2; ...
	zeros(Du), eye(Du), eye(Du)*h; ...
	zeros(Du), zeros(Du), eye(Du)];
Bt = [eye(Du)*h^3/6; eye(Du)*h^2/2; eye(Du)*h];
